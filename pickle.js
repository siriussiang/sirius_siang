...
genops: function(buff) {
 ...
 while(true) {
  ...
  if(typeof(opcode) === "undefined")throw new Error("Position "+(view.tell()-1)+": unknown opcode: "+code.charCodeAt(0));
  if (opcode.arg != null) {
...
load: function(buff){
 var stack=new pickle.linkedList(),memo = {},maxproto = -1,markstack = [],ops =this.genops(buff);
 for(var i in ops){
 ...
 return this.stack.popLast().value;
...
mark: function(stack){
 ...
 while(item = iter.prev())if(item instanceof pickle.marker)return iter;
 throw new Error("Mark not found");},
}

//Dummy class
pickle.marker=function(){};
pickle.marker.prototype.toString=function(){return "pickle.marker";}
//Used to mark basic types inside stack
pickle.container=function(value){this.value = value;}
...
pickle.view.prototype={
 tell: function() {return this.position;},
 seek: function(position) {this.position = position;},
 readline: function(ending){
  ...
  while(this.tell()<this.byteLength){
   ...
   if(c===ending)break;}
  return data;},
 getChar:function(){return String.fromCharCode(this.getUint8());},
 getChars:function(n){
  var str=[];
  for(var i=0;i<n;i++)str.push(this.getUint8());
  return String.fromCodePoint.apply(String,str);},
 getText:(function(){
  var decodeUTF8=TextDecoder.prototype.decode.bind(new TextDecoder('UTF-8'));
  return function(n){
   var pos=this.position;
   this.position+=n;
   return decodeUTF8(new Uint8Array(this.view.buffer,pos,n));}})(),
 getInt8:function(){
  var pos=this.position;
  this.position += 1;
...
pickle.readers={
 'read_uint1': function(view) {return view.getUint8();},
 'read_uint2': function(view) {return view.getUint16();},
 'read_int4': function(view) {return view.getInt32();},
 'read_stringnl': function(view,decode,stripquotes){
  ...
  if(decode)console.warn("String is currently not decoded.");
  return data;},
 'read_stringnl_noescape': function(view){return this.read_stringnl(view, false, false);},
 'read_stringnl_noescape_pair':function(view){return [this.read_stringnl(view,false,false),this.read_stringnl(view,false,false)];},
 'read_string4': function(view) {return view.getChars(this.read_int4(view));},
 'read_string1': function(view) {return view.getChars(this.read_uint1(view));},
 'read_unicodestringnl': function(view) {
  var data=view.readline();
  if(data.substr(data.length-1,1)!="\n")throw new Error("No newline found when trying to read unicode stringnl");
  console.warn('String may not be decoded');
  // unicode(data, 'raw-unicode-escape')
  return data.substr(0,data.length - 1);},
 'read_unicodestring4': function(view) {
  return view.getText(this.read_int4(view));},
 ...
 'read_floatnl': function(view) {return parseFloat(this.read_stringnl(view, false, false));},
 'read_float8': function(view) {return view.getFloat64();},
 'decode_long': function(long) {
  ...
  for(var i = 0; i <long.length - 1; i += 1) {
   ...
   h += hex;}
  n = parseInt(h, 16);
  if (long.charCodeAt(long.length - 1) == 0x80)n -= 1 <<(nbytes * 8);
  return n;
  //n = long(ashex, 16) #quadratic time before Python 2.3; linear now
  //if data[-1] >= '\x80':
  //n -= 1L << (nbytes * 8)
  },
 'read_long1':function(view){return this.decode_long(view.getChars(this.read_uint1(view)));},
 'read_long4':function(view){return this.decode_long(view.getChars(this.read_int4(view)));}}
...
'unicodestringnl':{ 'name':'unicodestringnl', n:pickle.UP_TO_NEWLINE,
reader:'read_unicodestringnl'},
'unicodestring4':{ 'name':"unicodestring4", n:pickle.TAKEN_FROM_ARGUMENT4, reader:'read_unicodestring4'},
'decimalnl_short':{ 'name':'decimalnl_short', n:pickle.UP_TO_NEWLINE,
reader:'read_decimalnl_short'},
'decimalnl_long':{ 'name':'decimalnl_long', n:pickle.UP_TO_NEWLINE,
reader:'read_decimalnl_long'},
'floatnl':{ 'name':'floatnl', n:pickle.UP_TO_NEWLINE, reader:'read_floatnl'},
'float8':{ 'name':'float8', n:8,
reader:'read_float8'},
...
'arg':null,
'proto':1,
'dispatch':function(stack, arg){ stack.push(new
pickle.container([]));}},
'a':{
'name':'APPEND',
'code':'a',
...
limit=mark.current();
while((item=mark.next())){
list.push(item.value);}stack.replace(limit, new pickle.container(list));}},
...
while((item=mark.next())){
list.push(item.value);
stack.remove(item);}stack.replace(limit, new pickle.container(list));}},
'\x85':{
'name':'TUPLE1',
'code':'\x85',
'arg':null,
...
if(count!=0)stack.remove(item);}stack.replace(item, new pickle.container(list));}},
'\x87':{
'name':'TUPLE3',
'code':'\x87',
'arg':null,
...
stack.remove(key);
stack.remove(value);
d[key.value]=value.value;}stack.replace(limit, new pickle.container(d));}},
's':{
'name':'SETITEM',
'code':'s',
'arg':null,
...
stack.remove(key);
stack.remove(value);}stack.remove(limit);}},
//Stack manipulation.
'0':{
'name':'POP',
'code':'0',
...
value=stack.last().value.apply(this,
arg.constructor===Array?arg:[arg]);
...
'proto':1,
'dispatch':function(stack, arg, memo){
throw new Error("Unsupported opcode BINPERSID");}}}//Used for stack
pickle.linkedList=function(){
this.id=pickle.linkedList.idCounter++;
this.iter=new
pickle.linkedList.iterator(this);}
...
/***Pushes new item to end of list**@param var content*@return object list item, used
for operation such as removing*/
push:function(obj){
var item={
previous:null,
next:null,
...
/***Inserts content after specified
item**@param object after item to be
inserted after*@param var
obj   content
to be inserted*@return object list item*/
...
return item;}//Update back reference
if(after.next!=null){
after.next.previous=item;}//Update next reference
after.next=item;
//Update ending reference
if(after==this.end){
this.end=item;}return item;},
/***Removes specified item from
list**@param object item(not
content!)*@return boolean true if item
...
this.end=null;
this.start=null;
return true;}if(item==this.end){
this.end=this.end.previous;
if(this.end!=null)this.end.next=null;
return true;}if(item==this.start){
this.start=item.next;
if(this.start!=null)this.start.previous=null;
return true;}item.previous.next=item.next;
item.next.previous=item.previous;
return true;},
/***Cycles throught all items.Callback is called with following arguments:*item content*item*item index**If callback returns anything
considered as true, cycle will be stopped.**@param function callback*/
...
str+="a("+val+"),";}else{
str+=val+",";}}if(str.substr(str.length-1, 1)==",")str=str.substr(0, str.length-1);
str+="]";
return str;},}
...
prev:function(){
if(this.point==null)this.point=this.ll.lastItem();
else
this.point=this.point.previous;
return this.current();},
next:function(){
if(this.point==null)this.point=this.ll.firstItem();
else
